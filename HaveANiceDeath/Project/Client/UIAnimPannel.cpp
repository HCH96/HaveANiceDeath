...
void UIAnimPannel::render_update()
{
	...
	ImGui::SameLine();
	if (ImGui::Button("Offset Setting", ImVec2(150.f, 30.f)))
	{
		m_vecOffset.clear();
...
void UIAnimPannel::OpenFileWindow()
{
	...
	ofn.lpstrFilter = L"ALL\0*.*\0texture\0*.png\0texture\0*.bmp";
	ofn.nFilterIndex = 1;
...
void UIAnimPannel::Compile()
{
	...
	// 저장한 오프셋의 개수와 선택된 프레임의 개수가 다른 경우
	if (m_vecOffset.size() != 0 && m_vecAnim.size() != m_vecOffset.size())
	{
		MessageBoxA(nullptr, "선택된 프레임의 개수와 Offset파일의 개수가 다릅니다.", "Compile Failed", MB_OK);
		return;
	}
	// 저장된 오프셋의 개수가 0이 아닌경우 -> 오프셋 등록
	else if(m_vecOffset.size() != 0)
	{
		// 저장
		for (size_t i = 0; i < m_vecAnim.size(); ++i)
		{
			m_vecAnim[i].vOffset = m_vecOffset[i];
		}
	}
...
void UIAnimPannel::ClearOffset()
{
	for (size_t i = 0; i < m_vecAnim.size(); ++i)
	{
		m_vecAnim[i].vOffset = Vec2(0.f, 0.f);
	}
...
{
	tAnimFrm anim = {};
	anim.Duration = 1.f / (float)m_DetailPannel->GetFPS();
	anim.vBackground = _vBackground;
	anim.vSlice = _vBackground;
	anim.vLeftTop = _vLeftTop;
	anim.vOffset = Vec2();
